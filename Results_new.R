...
geral <- geral %>% mutate(tempocap1 = ifelse(test = is.na(tempocap1),
                          ...
                          tempocap2 = ifelse(test = tempocap1==5400,
                                             ...
                                             no = ifelse(test = is.na(tempocap2), #Difference on two capture times
                                                yes = 5400-tempocap1,
                                                no = tempocap2)),
                          dif_temp_cap = tempocap2 - tempocap1)
...
#Interaction model
belo_temcap1_lme_int <- lmer(tempocap1 ~ ppsr*tratamento + (1|bloco),
...
#Simple model
belo_temcap1_lme <- lmer(tempocap1 ~ ppsr + tratamento + (1|bloco),
                         data = belostomatidae, na.action = na.omit, REML = F)
...
#Figure
belo_temcap1 <- model_line_semlog(belostomatidae, belostomatidae$ppsr,
                           (belostomatidae$tempocap1), 
